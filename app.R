...
# User Interface
ui <- fluidPage(
  titlePanel("Comparison between haircare products"),
  
  sidebarLayout(
    
    sidebarPanel(
      
      helpText("In this app we can confront four hair products from four different brands",
               "and see which brand carries the best product, both from the point of view of",
               "the price and of the cutomer reviews."),
      
      selectInput("prod_feat", label = "Select a product feature", choices = mylist)
...
server <- function(input, output) {
  ...
  ggplots <- reactive({
    if ( "Prices" %in% input$prod_feat) return(gg)
    if ( "Ratings" %in% input$prod_feat) return(gg1)
  })
  ...
  statements <- reactive({
    if ( "Prices" %in% input$prod_feat) return(lowest_priced_brand(prices))
    if ( "Ratings" %in% input$prod_feat) return(best_rated_brand(ratings))
  })
